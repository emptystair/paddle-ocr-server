...
Architecture: N worker threads share a single OCREngine (ONNX Runtime sessions are
thread-safe — the GIL is released during inference). Each thread gets its own
PDFRenderer (pypdfium2 is not thread-safe). Numpy arrays pass directly between
threads — no shared memory needed.
"""
...
CONFIG = {
    ...
    "NUM_ENGINES": int(os.getenv("NUM_ENGINES", "2")),
...
# ---------------------------------------------------------------------------
# PDF Renderer — per-thread instance (pypdfium2 is not thread-safe)
# ---------------------------------------------------------------------------

class PDFRenderer:
    def __init__(self, dpi: int, pdf_cache_size: int = 10):
        self.dpi = dpi
    ...
    def render_page(self, pdf_path: str, page_num: int, dpi: int = None) -> Optional[np.ndarray]:
        """Render a PDF page to a numpy array (RGB, uint8)."""
        try:
            ...
            max_pixels = CONFIG["MAX_PAGE_WIDTH"] * CONFIG["MAX_PAGE_HEIGHT"] * 3
            required = height * width * 3
            if required > max_pixels:
                fit = min(CONFIG["MAX_PAGE_WIDTH"] / width, CONFIG["MAX_PAGE_HEIGHT"] / height, 1.0)
                scale *= fit
                width = int(page.get_width() * scale)
                height = int(page.get_height() * scale)
            bitmap = page.render(scale=scale, rotation=0)
            img = bitmap.to_numpy()[:, :, :3].copy()
            return img
...
def page_ocr_worker(worker_id: int, engine: OCREngine, task_queue: queue.Queue,
                    result_queue: queue.Queue, stop_evt: threading.Event, config: dict):
    """Worker thread: pulls PageTasks, renders PDF pages, runs OCR, posts results."""
    renderer = PDFRenderer(config["DPI"], config["PDF_CACHE_SIZE"])
        ...
        while not stop_evt.is_set():
            try:
                ...
                if task is None:  # poison pill
                    break
                result = _process_page(worker_id, task, engine, renderer)
...
def _process_page(worker_id: int, task: PageTask, engine: OCREngine,
                  renderer: PDFRenderer) -> dict:
    """Render one page and run OCR. Returns a result dict."""
    t0 = time.time()
    try:
        img = renderer.render_page(task.pdf_path, task.page_num, task.dpi)
        ...
        det_shape = det_input_shape(img.shape[0], img.shape[1])
        ...
        if det_shape not in CACHED_DET_SHAPES:
            ...
            logger.info(f"W{worker_id} pg{task.page_num}: snapping det shape {det_shape} -> {snapped} for {task.pdf_path}")
        ...
        logger.debug(f"W{worker_id} pg{task.page_num}: render={t_render:.3f}s ocr={t_ocr:.3f}s lines={len(text_lines)}")
        result = {"job_id": task.job_id, "page_num": task.page_num, "pdf_path": task.pdf_path,
                  ...
                  "render_time": t_render, "ocr_time": t_ocr}
...
async def startup_event():
    global page_queue, result_queue, job_manager
    ...
    # Warm up TRT engines in the main process BEFORE creating the shared engine.
    # This compiles engines sequentially (no GPU contention) and caches them.
    # The shared OCREngine then loads pre-compiled engines from cache instantly.
    warmup_trt_engines(CONFIG)

    job_manager = JobManager()

    # Create shared OCREngine instances. Multiple engines reduce ORT session
    # mutex contention while still sharing the GPU. With N engines, idle memory
    # is ~N*3GB instead of ~21GB (8 separate processes).
    num_engines = CONFIG["NUM_ENGINES"]
    logger.info(f"Creating {num_engines} shared OCREngine(s)...")
    engines = [OCREngine(CONFIG) for _ in range(num_engines)]

    # cuDNN warmup on the first engine (all engines share the same GPU context)
    logger.info("Running cuDNN warmup...")
    t_warm = time.time()
    e = engines[0]
    dummy_det = np.random.randn(1, 3, 1408, 832).astype(np.float32)
    e.det_session.run(None, {e.det_input_name: dummy_det})
    dummy_cls = np.random.randn(1, 3, 48, 192).astype(np.float32)
    e.cls_session.run(None, {e.cls_input_name: dummy_cls})
    for w in [320, 480, 640, 960]:
        dummy_rec = np.random.randn(1, 3, 48, w).astype(np.float32)
        e.rec_session.run(None, {e.rec_input_name: dummy_rec})
    logger.info(f"cuDNN warmup done ({time.time()-t_warm:.1f}s)")

    page_queue = queue.Queue(maxsize=CONFIG["PAGE_QUEUE_SIZE"])
    ...
    for i in range(CONFIG["NUM_WORKERS"]):
        ...
        t = threading.Thread(
            target=page_ocr_worker,
            args=(i, engine, page_queue, result_queue, stop_event, CONFIG),
            daemon=True,
            name=f"OCRWorker-{i}",
        )
        t.start()
    ...
    logger.info(f"Server started with {CONFIG['NUM_WORKERS']} worker threads, "
                f"{num_engines} OCREngine(s)")
...
def queue_pdf_pages(job_id, pdf_paths, priority, dpi=None):
    ...
    for pdf_path in pdf_paths:
        try:
            ...
            for pg in pages:
                page_queue.put(PageTask(job_id, pdf_path, pg, n, priority, time.time(), dpi))
                queued += 1
        except Exception as e:
